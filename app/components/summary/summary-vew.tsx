...
const SectionTitle = ({ title }: { title: string }) => {
  return (
    <div className="w-full items-center flex flex-col gap-2 mb-6 sticky top-0 pt-2 backdrop-blur-sm  z-10">
      <h2 className="text-lg sm:text-3xl lg:text-4xl flex items-center gap-2 text-center font-bold">{title}</h2>
    </div>
  );
}
...
export function SummaryViewer({ summary }: { summary: string }) {
  ...
  const sections = summary
    ...
    .map(parseSection);
  return (
    <Card className="relative px-2 h-[500px] sm:h-[600px] lg:h-[700px] w-full xl:w-[600px] overflow-hidden bg-linear-to-br from-rose-50 to-rose-100 backdrop-blur-sm border border-rose-500/10 shadow-lg shadow-rose-500/20">
